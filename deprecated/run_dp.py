...
def computing_DP_on_one_instance(my_input, number_of_iterations):
    global dir_path_for_recap
    full_results_to_str = ""
    summary_to_str = ""
    list_time_values = []
    ...
    list_nb_of_generated_state = []
    for i in range (0,number_of_iterations):
        print("Computing the DP. Iteration i =", i+1)
        start_time = time.time()
        (Cmax, number_of_generated_states) = dynamic_programming(my_input)
        interval = time.time() - start_time
        list_sol_values.append(Cmax)
        list_time_values.append(interval)
        list_nb_of_generated_state.append(number_of_generated_states)
...
if __name__ == '__main__':
    ...
    dir_path_for_results = "C:\\Users\\sarah\\Documents\\These\\Sub-problems\\2__tree-merging__Cmax\\3-Programmation\\Results\\DP\\" + str(number_of_tiles) + "_tiles"
