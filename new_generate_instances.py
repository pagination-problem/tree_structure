...
class InstanceMaker:
    ...
    def compute_coefficient_for_quadratic_var(self, the_instance):
        try:
            ...
            # Traitement du reste de la matricabse :
            for (new, new_tile) in enumerate(the_instance.tiles):
                for last_tile in the_instance.tiles[:new]:
    ...
    def create_weight_according_to_mean_and_deviation(
        ...
        # in this document line ?? : cfg["min_symbol_weight_bound"], cfg["max_symbol_weight_bound"]
...
if __name__ == "__main__":
    filename = "instances/sarah/test_new_config.json" if len(sys.argv) <= 1 else sys.argv[1]
